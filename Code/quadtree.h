...
class QuadNode {
   private:
    void findPointLeaf(point2d p, vector<GPSdata*>& leaf) const;
    void collectRange(const Rectangle& rect, vector<GPSdata*>& out) const;
   ...
   public:
    ...
    QuadNode* Search(point2d p) const;
    bool isLeaf() const;

    vector<GPSdata*> PointSearch(point2d p) const;
    vector<GPSdata*> AreaSearch(const Rectangle* rect) const;
    vector<GPSdata*> TrajectorySearch(const Rectangle* rect, int target_idx) const;
